...
TEST_OBSERVATION_KEYS = ("chars", "message", "pixel")
...
#message vector values used for reward shaping + subtasks
...
def positions(chars):
    ...
    goal_pos = np.where(chars == GOAL_ID)
...
def test_policy(env, actor_critic, savedir, deterministic=True):
    ...
    levitating = False
    raw_state = env.reset()
    PIXEL_HISTORY = [raw_state["pixel"][80:260, 490:740]]
    ...
    while True:
        ...
        PIXEL_HISTORY.append(next_state["pixel"][80:260, 490:740])
    ...
    while True:
        ...
        for _ in range(BATCH_SIZE):
            ...
            if np.array_equal(next_state["message"], DRINK_LEVITATION_POTION_MESSAGE):
                reward += DRINK_REWARD
                levitating = True
            elif (not np.array_equal(next_state["message"], QUAFF_POTION_MESSAGE)) and chosen_actions == 5:
                reward += THROW_PENALTY
                terminated = True
